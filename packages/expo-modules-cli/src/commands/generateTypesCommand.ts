...
const DEFAULT_OUTPUT_PATH = path.join('src', 'native.d.ts');
  ...
  cli
    ...
    .description(
      'Generates TypeScript declarations for the native API an Expo module exports to JavaScript: ' +
        ...
        'current directory) for Swift sources and writes them to a single .d.ts file.'
    )
    .option(
      '-o, --out <path>',
      'The declaration file to write, relative to the package directory.',
      DEFAULT_OUTPUT_PATH
    )
    .option(
      '--scanner <path>',
    ...
    .action(async (packageDirArg: string | undefined, options: GenerateTypesCommandOptions) => {
      ...
      console.log(
        `Generated ${styleText('bold', displayPath(result.outputPath))}: ` +
          ...
          `${count(records, 'record')} from ${count(result.stats.filesParsed, 'Swift file')}.`
      );
    });
}
